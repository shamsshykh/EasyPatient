...
class MealPlanAdapter(
    ...
    private val onClickAction : (MealPlanModel, Boolean) -> Unit
) : RecyclerView.Adapter<MealPlanAdapter.ViewHolder>() {
    private val mealPlanListFilter = ArrayList<MealPlanModel>()
    var archive: Boolean
    var progressDialog: ProgressDialog? = null
    var networkChecker: NetworkChecker
    private val db: EasyPatientDatabase
    private val mMealPlanDetailDao: MealPlanDetailDao
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val listData = mealPlanList[position]
        if (archive) {
    ...
    private fun postArchiveStatus(position: Int) {
        ...
        if (archive) {
            val dialog =
                ArchiveBottomSheetDialog(ArchiveBottomSheetDialog.MEAL_PLAN_TYPE,
                    object : ArchiveCallback {
                        override fun performArchive() {
                            OrientationArchiveResponse =
                                api.putMealPlanAvailable(mealPlanList[position].id)
                            request(position)
                        }
                    })
            dialog.show(
            ...
            OrientationArchiveResponse = api.postMealPlanArchive(mealPlanList[position].id)
            request(position)
        }
    }
        ...
        OrientationArchiveResponse!!.enqueue(object : Callback<StatusModel> {
            override fun onResponse(call: Call<StatusModel>, response: Response<StatusModel>) {
                if (response.isSuccessful) {
                    ...
                    if (status) {
                        if (status) {
                            val model = mealPlanList[position]
                            if (archive) {
                                EasyPatientDatabase.databaseWriteExecutor.execute {
                                    mMealPlanDetailDao.insertMealPlanItem(
                                        model
                                    )
                                }
                                ...
                                EasyPatientDatabase.databaseWriteExecutor.execute {
                                    mMealPlanDetailDao.deleteMealPlanItem(
                                        model.id
                                    )
                                }
                        ...
                        mealPlanList.removeAt(position)
                        notifyDataSetChanged()
                        listUpdate.updateUI(mealPlanList.size)
                    } else Toast.makeText(
                        context,
    ...
    fun filter(charText: String) {
        ...
        if (charText.length == 0) {
            mealPlanList.addAll(mealPlanListFilter)
        } else {
            for (wp in mealPlanListFilter) {
    ...
    init {
        mealPlanListFilter.addAll(mealPlanList)
